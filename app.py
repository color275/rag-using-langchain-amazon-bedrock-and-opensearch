...
def main():
    ...
    pdf_file = st.sidebar.file_uploader("ChatBot이 참고할 PDF 포맷의 문서 업로드", type=["pdf"], key=None)
    ...
    if pdf_file is not None and pdf_file != st.session_state.last_uploaded:
        ...
        progress_text.text("OpenSearch에 벡터정보 저장 중")
    ...
    with st.form("my-form"):
        question = st.text_input("질문을 입력하세요:", value=st.session_state.get('question', ''), key="question")
    ...
    if st.session_state.qa_history:
        for q, a in st.session_state.qa_history:
            st.markdown(f"<p style='color: #32CD32;'><b>{q}</b></p>", unsafe_allow_html=True)
